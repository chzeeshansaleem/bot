    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            if captcha_solution is not None:
                ...
                page.goto('https://blsitalypakistan.com/bls_appmnt/bls-italy-appointment/NFR4elVyMTIxNDgzNTIwNjM')
                ...
                disabled_dates = page.query_selector_all('.datepicker-days tbody td.label-available')
                for date in disabled_dates:
                    day = date.inner_text()
                    print(f'Disabled date: {day}')
                
                # Find the first available date
                available_date = page.query_selector('.datepicker-days tbody td:not(.label-available):not(.old):not(.new):not(.label-slotfull)')
                if available_date:
